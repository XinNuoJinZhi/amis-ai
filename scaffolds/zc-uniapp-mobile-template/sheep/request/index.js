...
import { getTenantId, getAppTenantCode, removeToken } from "@/sheep/util/auth"
import { protocolHandle } from "@/sheep/util/protocol" 
...
http.interceptors.request.use(
  (config) => {
    const obj = protocolHandle(config.url, config.method)
    ...
    const appTenantCode = getAppTenantCode();
    if (appTenantCode) {
      config.header['App-Tenant-Code'] = appTenantCode;
    }
    ...
    config.header['env'] = env;
    if(uni.getStorageSync('portalVal')) {
      config.header['portal-key'] = uni.getStorageSync('portalVal');
    }
    if (_appid) {
      config.header['Target-App-Id'] = _appid
    }
    if (_tenantId) {
      config.header['Target-Tenant-Id'] = (_tenantId == -1 ? tenantId : _tenantId)
    }
    return config;
  },
  (error) => {
...
http.interceptors.response.use(
  (response) => {
    ...
    if(response.config.url.indexOf('/system/captcha/get') > -1 || response.config.url.indexOf('/system/captcha/check') >-1) {
