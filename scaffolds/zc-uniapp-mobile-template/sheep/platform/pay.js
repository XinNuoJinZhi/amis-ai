...
let sheep;
const getSheep = () => {
  if (!sheep) sheep = require('@/sheep').default;
  return sheep;
};
// #ifdef H5
...
export default class SheepPay {
  ...
  payAction() {
    ...
    return payAction[getSheep().$platform.name][this.payment]();
  }
  ...
  prepay(channel) {
    return new Promise(async (resolve, reject) => {
      ...
      if (['wx_pub', 'wx_lite'].includes(channel)) {
        const openid = await getSheep().$platform.useProvider('wechat').getOpenid();
  ...
  async copyPayLink() {
    ...
    uni.showModal({
      ...
      success: (res) => {
        if (res.confirm) {
          getSheep().$helper.copyText(data.displayContent);
        }
      },
    });
  }
  ...
  bindWeixin() {
    uni.showModal({
      ...
      success: function (res) {
        if (res.confirm) {
          getSheep().$platform.useProvider('wechat').bind();
        }
      },
    });
  }
...
export function getPayMethods(channels) {
  ...
  const platform = getSheep().$platform.name;
...
export function goPayResult(id, orderType, resultType) {
  getSheep().$router.redirect('/pages/pay/result', {
    id,
    orderType,
    payState: resultType,
  });
