        ...
        >
          <template #right>
            <button
              ...
              @tap="getSmsCode('changePassword', mobile)"
            >
...
<script setup>
  ...
  const mobile = JSON.parse(uni.getStorageSync('user-store')).userInfo.user.mobile
  // 数据
  const state = reactive({
    model: {
      mobile: JSON.parse(uni.getStorageSync('user-store')).userInfo.user.mobile, // 手机号
      code: '', // 验证码
      password: '', // 密码
  ...
  async function changePasswordSubmit() {
    ...
    // 发起请求
    state.model.captcha = state.model.code
