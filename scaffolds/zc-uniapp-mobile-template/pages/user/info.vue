...
<template>
  <s-layout title="用户信息" class="set-userinfo-wrap">
    ...
    >
        ...
        <uni-forms-item name="sex" label="性别">
          <view class="ss-flex ss-col-center ss-h-100">
            <radio-group @change="onChangeGender" class="ss-flex ss-col-center">
              <label class="radio sex ss-m-r-32" v-for="item in sexRadioMap" :key="item.value">
      ...
      <!--个人信息-->
      <view class="bg-white ss-p-x-30 ss-m-t-14">
        <uni-list>
          <uni-forms-item name="username" label="用户名称">
            <text class="personal">{{state.model.username}}</text>
          </uni-forms-item>
          <!-- <uni-forms-item name="mobile" label="手机号码">
            <text>{{state.model.mobile}}</text>
          </uni-forms-item>
          <uni-forms-item name="email" label="用户邮箱">
            <text>{{state.model.email}}</text>
          </uni-forms-item> -->
          <uni-forms-item name="deptName" label="所属部门">
            <text class="personal">{{state.model?.dept?.name}}</text>
          </uni-forms-item>
          <uni-forms-item name="postName" label="所属岗位">
            <text class="personal">{{state.model?.posts?.map((post) => post.name).join(',')}}</text>
          </uni-forms-item>
          <uni-forms-item name="roles" label="所属角色">
            <text class="personal">{{state.model?.roles?.map((role) => role.name).join(',')}}</text>
          </uni-forms-item>
          <uni-forms-item name="createTime" label="创建日期">
            <text class="personal">{{dayjs(state.model?.createTime).format('YYYY-MM-DD HH:mm:ss')}}</text>
          </uni-forms-item>
        </uni-list>
      </view>
    </uni-forms>
...
<script setup>
  ...
  const userInfo = computed(() => sheep.$store('user').userInfo.user);
  // 选择性别
