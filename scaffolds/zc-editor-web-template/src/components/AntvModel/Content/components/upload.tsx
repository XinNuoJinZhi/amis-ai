...
import {getListAllSimple} from '@/api/antv';
import {form_field_type_options} from '@/bpmn/panel/ElementForm/dataSelf';
...
const Upload = forwardRef(function Upload(props, ref) {
  ...
  useEffect(() => {
    ...
    form.setFieldValue('configCode', needValue.configCode);
    form.setFieldValue('paramName', needValue.paramName);
    form.setFieldValue('outputName', needValue.outputName);
    setTargetNameValue(needValue.outputName);
    needValueData.current = needValue;
    getListAllSimple().then(res => {
      setConfigOptions(res.data.data);
    });
    setAppTenantCodeValue(needValue.paramName);
    setShowForn(false);
    setTimeout(() => {
      setShowForn(true);
    }, 100);
      ...
      >
        <Collapse defaultActiveKey={['1']} expandIconPosition={'end'}>
          <Collapse.Panel header={'基本信息'} key={1}>
            ...
            <Form.Item<FieldType> label="对象存储" name="configCode">
              <Select
                onChange={configCodeChange}
                placeholder={'请选择'}
              >
                {configOptions.map((e: any) => {
                  return (
            ...
            <Form.Item<FieldType> label="文件地址" name="paramName">
