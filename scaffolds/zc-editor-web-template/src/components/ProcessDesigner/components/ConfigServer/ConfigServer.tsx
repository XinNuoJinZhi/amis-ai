import { ACTIVITI_PREFIX, CAMUNDA_PREFIX, FLOWABLE_PREFIX } from '@/bpmn/constant/constants';
import { defaultThemeData } from '@/components/ProcessDesigner/globalTheme';
...
import { Button, Drawer, FloatButton, Form, Radio, Space, Switch, message } from 'antd';
...
export default function ConfigServer() {
  ...
  function handleDark() {
    if (darkMode) {
      ...
      document.body.style.setProperty('--canvas-bg-color', defaultThemeData.lightCanvasBgColor);
      ...
      document.body.style.setProperty('--canvas-bg-color', defaultThemeData.darkCanvasBgColor);
  ...
  return (
    <>
      <FloatButton.Group icon={<ToolOutlined />} type="primary" trigger="hover">
        ...
        <FloatButton
          icon={darkMode ? <BulbFilled style={{ color: '#ffe700' }} /> : <BulbOutlined />}
          onClick={handleDark}
        />
        {/* <FloatButton icon={<QuestionOutlined />} onClick={() => message.info('暂未开发')} /> */}
      </FloatButton.Group>
      <Drawer
        title="配置中心"
        placement={'left'}
        ...
        <Form labelCol={{ span: 5 }} wrapperCol={{ span: 20 }} form={form}>
          {/* <Form.Item label="流程引擎" name="engineType">
            <Radio.Group onChange={(e) => changeEngineType(e.target.value)}>
              <Radio.Button value={`${CAMUNDA_PREFIX}`}>{`${CAMUNDA_PREFIX}`}</Radio.Button>
              <Radio.Button value={`${FLOWABLE_PREFIX}`}>{`${FLOWABLE_PREFIX}`}</Radio.Button>
              <Radio.Button value={`${ACTIVITI_PREFIX}`}>{`${ACTIVITI_PREFIX}`}</Radio.Button>
            </Radio.Group>
          </Form.Item> */}
          <Form.Item valuePropName="color" name="primaryColorObj" label="主题换肤">
            <SketchPicker
              onChange={(color: any) => {
